...
Logger::Logger() : level_(LINFO), lastRotate_(time(NULL)), rotateInterval_(86400), rotateSize_(1024 * 1024 * 1) {
    tzset();
    fd_ = -1;
    realRotate_.store(lastRotate_);
}
...
void Logger::setFileName(const string& filename) {
    ...
    if(access(file_path.c_str(), F_OK) != 0 && mkdir(file_path.c_str(), 0755) == 0) {
        fprintf(stdout, "mkdir success\n");
    } else {
        fprintf(stdout, "file path exist\n");
    }
...
void Logger::maybeRotate() {
    ...
    //     return;
    // }
    if(lseek(fd_, 0, SEEK_END) < rotateSize_) {
        return;
    }
    static int index = 0;
    if(index == 5) index = 0 ;
    ...
    // localtime_r(&now, &ntm);
    char newname[4096];
    // snprintf(newname, sizeof(newname), "%s.%d%02d%02d%02d%02d", filename_.c_str(), ntm.tm_mon + 1, ntm.tm_mday, ntm.tm_hour, ntm.tm_min, ntm.tm_sec);
    snprintf(newname, sizeof(newname), "%s.%d", filename_.c_str(), index++);
    const char *oldname = filename_.c_str();
    int err = rename(oldname, newname);
    if (err != 0) {
        fprintf(stderr, "rename log file %s -> %s failed msg: %s\n", oldname, newname, strerror(errno));
        return;
    }
